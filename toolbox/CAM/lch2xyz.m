function xyz = lch2xyz(lch,whitepoint,exp)
...
%   disp('You should really provide your own white point data to ensure reasonable results.');
  whitepoint = [91.55 100 101.52]; % Custom mode white point
end
...
m = size(lch);
lch = reshape(lch, prod(m)/3, 3);
a = lch(:,2).*cos(lch(:,3).* 2 .*pi./360);
b = lch(:,2).*sin(lch(:,3).* 2 .*pi./360);
